...
export function WorkflowCard({ workflow, onMutated }: WorkflowCardProps) {
  ...
  return (
    <div className="relative">
        ...
        <Card className="hover:bg-accent/50 transition-colors cursor-pointer h-full">
          <CardHeader className="pb-2">
            <div className="flex items-start justify-between gap-2">
              ...
              {isAdmin && onMutated && <div className="h-7 w-7 shrink-0" />}
            </div>
      ...
      {isAdmin && onMutated && (
        ...
        >
          <WorkflowActionsMenu workflow={workflow} onMutated={onMutated} />
        </div>
      )}
    </div>
  )
